...
def test_mse_loss(x, t):
    ...
    ac_y = F.mse_loss(ac_x, ac_t)
    torch_y = tF.mse_loss(torch_x, torch_t)
...
def test_cross_entropy_loss(x, t):
    ...
    ac_y = F.cross_entropy(ac_x, ac_t)
    permutation = (0, ac_x.ndim - 1) + tuple(d for d in range(ac_x.ndim - 1) if d > 0)
    torch_y = tF.cross_entropy(torch_x.permute(*permutation), torch_t)
    _loss_function_verify(ac_x, torch_x, ac_y, torch_y)
