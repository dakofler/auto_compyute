...
class ScaledDotProductAttention(Op):
    ...
    def forward(
        ...
        *_, seq_len, head_size = q.shape
        ...
        self.save_to_cache(q, k, v, drop_attnw, p, drop_mask)
        return y

    def backward(self, dy: ArrayLike) -> tuple[ArrayLike, ...]:
        q, k, v, drop_attnw, p, drop_mask = self.retrieve_from_cache()
        head_size = q.shape[-1]
        ...
        dq = dattn_weights @ k
        dk = dattn_weights.swapaxes(-1, -2) @ q
        dv = drop_attnw.swapaxes(-1, -2) @ dy
